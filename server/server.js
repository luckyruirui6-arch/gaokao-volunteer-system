...
const POLICY_DIR = path.join(REPO_ROOT, "data", "policy");
const MAJOR_DIR = path.join(REPO_ROOT, "data", "majors");
const PROVINCE_DIR = path.join(REPO_ROOT, "data", "provinces");
const SCHOOL_DIR = path.join(REPO_ROOT, "data", "schools");
const SCORE_DB_PATH = path.join(REPO_ROOT, "data", "scores", "gaokao_2025.db");
const SCORE_DB_MODULE_PATH = path.join(REPO_ROOT, "node_modules", "better-sqlite3");
...
function buildScoreInference(prompt, routeResult) {
  ...
  if (!db) {
    return {
      context: "录取数据库当前不可用，本轮先按知识库文档和通用经验做分析。",
      hits: [],
  ...
  const linesText = [];
  linesText.push(`数据库命中情况：${province || "未识别省份"}${school ? `，学校 ${school}` : ""}${major ? `，专业 ${major}` : ""}。`);
  if (lineRow) {
  ...
  linesText.push("注意：以上都是基于历史数据做的简单倾向判断，只能看趋势，不能等同于你今年最终录取结果。");
  ...
  if (lineRow) {
    hits.push({
      ...
      score: 130,
      title: `录取数据/${lineRow.province}_控制线_${lineRow.year}`,
      path: `录取数据/gaokao_2025.db#province_lines`,
      summary: `${lineRow.province} ${lineRow.year} ${lineRow.batch} ${lineRow.subject} 控制线 ${lineRow.score_line}`,
    });
  }
  if (representative) {
    hits.push({
      collection: "gaokao_score_rules",
      score: 129,
      title: `录取数据/${representative.school}${representative.major ? `_${representative.major}` : ""}_${representative.province}_${representative.year}`,
      path: `录取数据/gaokao_2025.db#${representative.major ? "major_scores" : "school_scores"}`,
      summary: `${representative.province} ${representative.year} ${representative.school}${representative.major ? ` ${representative.major}` : ""}${representative.batch ? ` ${representative.batch}` : ""}${representative.subject ? ` ${representative.subject}` : ""}，最低分 ${representative.min_score ?? "待补"}，最低位次 ${representative.min_rank ?? "待补"}`,
    });
  }
  ...
  if (routeResult.route === "policy_consult" || routeResult.route === "score_tendency") {
    for (const province of provinceNames) {
      ...
      for (const file of files) {
        const collection = file.includes(`${path.sep}data${path.sep}policy${path.sep}`) ? "gaokao_policies_rules" : "gaokao_province_data";
...
function classify(prompt) {
  ...
  if (/(大学|学院|学校)/.test(text) && /(怎么样|好不好|怎么样|介绍一下|评价|推荐)/.test(text)) {
    return { 
      route: "school_consult",
      priorityCollections: ["gaokao_schools"],
      fallbackCollections: ["gaokao_style_cases"]
    };
  }
  
  if (/(专业|学什么|前景|就业|好不好)/.test(text)) {
    return { 
      route: "major_consult",
      priorityCollections: ["gaokao_majors"],
      fallbackCollections: ["gaokao_style_cases", "gaokao_schools"]
    };
  }
  
  if (/(政策|规则|志愿|投档|录取|位次|分数)/.test(text)) {
    return { 
      route: "score_tendency",
      priorityCollections: [],
      fallbackCollections: ["gaokao_province_data", "gaokao_style_cases"]
    };
  }
  
  if (/(赋分|选科|提前批|专项|公费|定向)/.test(text)) {
    return { 
      route: "policy_consult",
      priorityCollections: ["gaokao_policies_rules", "gaokao_province_data"],
      fallbackCollections: ["gaokao_style_cases"]
    };
  }
  
  return { 
    route: "family_emotion",
    priorityCollections: [],
    fallbackCollections: ["gaokao_style_cases", "gaokao_majors"]
  };
}
...
function buildSystemPrompt(config) {
  ...
  const hardStyle = [
    ...
    "2. 不要端着，不要百科腔，不要空泛安慰。",
    "3. 风格可以直接，但事实必须贴着数据走；没有数据时要明确提醒用户自行查询官方信息。",
    "4. 涉及录取可能性判断时，位次优先于分数；如果用户只给分数没有位次，要明确提醒：这轮只能粗判断，真正想判断得更准，需要补充位次。"
  ].join("\n");
  return `${base}\n\n${hardStyle}`;
}
...
async function answerQuestion(prompt, config) {
  ...
  const answer = await callOpenAICompatible(prompt, routeResult, hits, config, scoreInference.context);
...
server.listen(PORT, HOST, () => {
  console.log(`Gaokao Volunteer System started: http://127.0.0.1:${PORT}`);
  console.log(`LLM: ${config.baseUrl}`);
  console.log(`Qdrant: ${config.qdrantUrl || "not configured"}`);
});
